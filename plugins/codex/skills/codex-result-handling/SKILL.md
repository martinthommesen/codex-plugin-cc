...
- For `codex:codex`, do not turn a failed or incomplete Codex run into a Claude-side implementation attempt. Report the failure and stop.
- For `codex:codex`, if Codex was never successfully invoked, do not generate a substitute answer at all.
- For `ask` advisor output, present the answer as a peer opinion — reconcile it with your own view explicitly — and keep the trailing advisor-thread line so the user can continue the conversation.
